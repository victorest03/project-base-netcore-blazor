namespace SistemaVentas.Frontend.Helper
{
    ...
    public static class EditContextFluentValidationExtensions
    {
        ...
        private static void ValidateField(EditContext editContext, ValidationMessageStore messages, in FieldIdentifier fieldIdentifier)
        {
            ...
            foreach (var validationResult in validationResults.Errors)
            {
                if (messages[editContext.Field(validationResult.PropertyName)].ToList().Count == 0)
                {
                    messages.Add(editContext.Field(validationResult.PropertyName), validationResult.ErrorMessage);
                }
            }
        ...
        private static IValidator GetValidatorForModel(object model)
        {
            ...
            var modelValidatorType = model.GetType().Assembly.GetTypes().FirstOrDefault(t => t.IsSubclassOf(abstractValidatorType));
            var modelValidatorInstance = (IValidator)Activator.CreateInstance(modelValidatorType ?? throw new InvalidOperationException());
